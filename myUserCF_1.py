...
############################################
def Recall(train, test, user_rate_dic, item_to_user, K, N):
	hit = 0
	all = 0
#	print train
	for user in train.keys():
		if user not in test.keys():
			continue
		tu = test[user]
		rank = GetRecommendation(user, user_rate_dic, item_to_user, K, N)
...
#############################################
def Precision(train, test, user_rate_dic, item_to_user, K, N):
	hit = 0
	all = 0
	for user in train.keys():
		if user not in test.keys():
...
#    N: number itmes in recommendation list #
#############################################
def Coverage(train, test, user_rate_dic, item_to_user, K, N):
	recommend_items = set()
	all_items = set()
	for user in train.keys():
...
#    N: number itmes in recommendation list #
#############################################
def Popularity(train, test, user_rate_dic, item_to_user, K, N):
	item_popularity = dict()
	for user, items in train.items():
		for item in items:
			if item[0] not in item_popularity:
	...
	for key1 in user1:
		for key2 in user2:
			if key1[0] == key2[0]:
				...
				sum_xy += 1 / math.log(1 + (key1[1] - avg_x) * (key2[1] - avg_y))
...
def GetRecommendation(user, user_rate_dic, item_to_user, K, N):
	neighbors = CalcNearestNeighbor(user, user_rate_dic, item_to_user)[:K]
	...
	for neighbor in neighbors:
		...
		items = user_rate_dic[neighbor_user_id]
		for item in items:
			if item[0] not in recommend_dic:
				recommend_dic[item[0]] = neighbor[0] * item[1]
			else:
				recommend_dic[item[0]] += neighbor[0] * item[1]
	rank = sorted(recommend_dic.items(), key=operator.itemgetter(1), reverse=True)[:N]
	user_items = [i[0] for i in user_rate_dic[user]]
	return [[i[0], i[1]] for i in rank]
...
if __name__ == '__main__':
	...
	for i in range(M):
		...
		#print "train ok\n"
		user_rate_dic, item_to_user = CreateUserRankDic(train)
		recall += Recall(train, test, user_rate_dic, item_to_user, K, N)
		precision += Precision(train, test, user_rate_dic, item_to_user, K, N)
		coverage += Coverage(train, test, user_rate_dic, item_to_user, K, N)
		popularity += Popularity(train, test, user_rate_dic, item_to_user, K, N)
